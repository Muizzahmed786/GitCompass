  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-blue-600 text-white shadow-xl flex items-center justify-center hover:bg-blue-700 transition-all hover:scale-105 active:scale-95 cursor-pointer"
      >
        {isOpen ? (
      ...
      {/* Floating Chat Modal */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-3rem)] h-[520px] bg-surface rounded-2xl shadow-2xl border border-divider flex flex-col overflow-hidden animate-fade-in">
          {/* Header */}
          <div className="p-3.5 bg-surface border-b border-divider flex items-center justify-between shadow-2xs">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-text-primary text-sm leading-none">AI Architect Chat</h3>
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                title="Close chat"
                className="p-1.5 rounded text-text-tertiary hover:text-text-primary hover:bg-surface-hover transition-colors ml-1"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Messages */}
          <div ref={scrollRef} className="flex-1 p-4 overflow-y-auto space-y-4 bg-surface-hover/20">
          ...
          <div className="p-3 border-t border-divider bg-surface">
            <form onSubmit={handleSend} className="relative">
              <input
                ...
                className="w-full bg-surface-hover border border-divider rounded-full pl-4 pr-10 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-colors"
              />
              <button
                type="submit"
                disabled={!input.trim() || loading}
                className="absolute right-1.5 top-1.5 w-7 h-7 bg-blue-600 text-white rounded-full flex items-center justify-center disabled:opacity-40 hover:bg-blue-700 transition-colors"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
