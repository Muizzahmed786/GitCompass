...
from app.services.miner import mine_repository_task, sync_repository_task
...
@router.post("/{repo_id}/sync", response_model=RepositoryResponse, status_code=status.HTTP_202_ACCEPTED)
async def sync_repository(
    ...
    """Trigger incremental background sync for new commits in repository."""
    user_id = user["sub"]
    repo_res = db.table("repositories").select("*").eq("id", repo_id).execute()
    ...
    logger.info("Enqueued incremental sync for repository %s", repo_id)
    return repo
